...
void
Ray::changeDirection(const Point & direction)
{
  if (direction.absolute_fuzzy_equals(Point(0, 0, 0)))
    mooseError("Cannot set zero vector direction for a Ray\n\n", getInfo());

  _direction = direction.unit();
  // just ignoring the change in direction for now
  // setting this true breaks PIC when modifying the direction on
  // everytime step
  // We don't need to keep track of if a ray has changed direction in
  // PIC anyways
  // _trajectory_changed = true;
}
